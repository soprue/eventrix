...
export default function transformEventDataToFormValues(
  ...
  return {
    ...
    registrationStartDate:
      eventData.registrationStart instanceof Date
        ? eventData.registrationStart
        : eventData.registrationStart.toDate(),
    registrationStartTime: formatTime(eventData.registrationStart),
    registrationEndDate:
      eventData.registrationEnd instanceof Date
        ? eventData.registrationEnd
        : eventData.registrationEnd.toDate(),
...
function formatTime(date: Date | Timestamp): string {
  const d = date instanceof Date ? date : (date.toDate() as Date);
